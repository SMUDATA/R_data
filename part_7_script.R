...
aaa <- bind_cols(order_info_r, tmp_order_info_r)

aaa <- bind_cols(order_info_r, reservation_r)

tmp_order_info_r <- order_info_r
aaa <- bind_rows(order_info_r, tmp_order_info_r)

aaa <- bind_rows(order_info_r, reservation_r)
...
aaa <- full_join(reservation_r, table_added_row, by = "reserv_no") %>% 
    arrange(reserv_no, item_id) %>% 
    select(reserv_no, customer_id, visitor_cnt, cancel, order_no, item_id, sales)
