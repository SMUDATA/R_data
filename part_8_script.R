...
ggplot(df_branch_sales_2, 
       aes(x = reorder(branch, -amt), y = amt)) + 
    geom_bar(stat = "identity")

ggplot(df_branch_sales_2, 
       aes(x = reorder(branch, -amt), 
           y = amt, 
           fill = branch)) + 
    geom_bar(stat="identity")
...
# 수동으로 색상을 채움
gg <- gg + 
    scale_fill_manual(values = c("STEAK" = "red", 
                                 "SPECIAL_SET" = "orange", 
                                 "SEA_FOOD" = "skyblue", 
                                 "SANDWICH" = "skyblue", 
                                 "SALAD_BAR" = "skyblue", 
...
ggplot(total_amt, 
       aes(x = month, y = amt_monthly, group = 1, 
           label = amt_monthly)) + 
